...
// $m = new SmtpClient('user6@hotmail.com','Password');
$m = new SmtpClient();
...
$m->addHostname("smtp.live.com");
// Add smtp port
$m->addPort(25);
// Smtp server email
$m->addUser('email@hotmail.com');
// Smtp server password
$m->addPassword("password");

// Add from
$m->addFrom("email@hotmail.com", "Ania Bez pic");
// Add to
$m->addTo("email@email.com", "Maxiu");
// Add to
// $m->addTo("hello@domain.com", "Maxiu");

// Add Cc
$m->addCc("email@gmail.com", "No i działa");

// Add Bcc
$m->addBcc("email@domain.com", "Yahoo email");//
...
// Message subject
$m->addSubject(" - Php smtp email client!");
...
// $m->createMime("Witaj!",'<h1>Witaj Maniu <img src="cid:zenek123"> </h1>',"Wesołych świąt życzę!","Heniek Wielki", "hen@domain.xx");
...
// Send email without smtp server authentication
function SendEmail($email, $msg, $subject = 'Aktywacja konta', $from_user = "SMS Newsletter", $from_email = "email@email.com"){
    // require('smtp.php');
    // Create object
    $m = new Smtp();
    // $m->Debug(1);
    ...
    // Smtp server hostname
    $m->addHostname("localhost");
    // Add smtp port
    $m->addPort(25);

    $m->addFrom($from_email, $from_user);
    $m->addTo($email);
    $m->addText($msg);
    $m->addHtml($msg);
    ...
    if($ok == 0){
        // Show last error
        echo '<p style="color: #05152d; background: #eee; padding: 10px; float: left; width: 100%; text-align: center">'.$m->lastError.'</p>';
    }
    return $ok;
}

// Send email with smtp server authentication
function SendEmailAuth($email, $msg, $subject = 'Aktywacja konta', $from_user = "SMS Newsletter", $from_email = "email@email.com"){
    // require('smtp.php');
    // Create object
    $m = new Smtp();
    // $m->Debug(1);
...
// echo SendEmail('xxxx@gmail.com', "Hello test without auth localhost");
?>
